...
**先看领域全景，再沿章节逐步补全。** D01～D17 建立大模型全貌，D18～D32 深入推理工程原理；你已反馈读完 D32。后续主线进入 D33～D47 推理工程实战，Agent Harness 支线继承旧项目的 Learn Claude Code 课程，从 s11 继续。读完、完成实验和通过验收分别记录在[学习进度](progress.md)。
...
flowchart TB
    ...
    future --- agentdeep["智能体工程<br/>Runtime · 可靠执行 · 安全评测"]
```
...
flowchart TB
    ...
    moe["MoE 推理专题<br/>专家路由与通信"]
    lcc["Agent Harness 工程<br/>LCC s01—s17<br/>继承 s01—s10，下一章 s11"]
    infer1["D33—D39 推理实战前半<br/>环境、基线、服务、压测与诊断"]
    infer2["D40—D47 推理实战后半<br/>优化、多卡、可靠性与交付"]
    a --> b --> c --> d --> e --> f --> g --> h --> i --> j --> k --> infer1 --> infer2
    k -.->|模型扩展专题| moe
    f -.->|工程实战支线| lcc
```

实线表示课程主线，虚线表示专题或工程支线。LCC 从 D12、D13 的工具调用与 Agent 基础继续进入可运行的 Harness 代码，继承的“已学习”记录不自动代表掌握或当前环境复现成功。每章对应一个问题。**D01～D17 的正文、助记卡、自测与参考答案均已生成；资料已生成不代表已经阅读或掌握。**
...
| [**MoE 专题**](courses/02-inference/moe-special-topic/README.md) | 为什么参数很多，却只计算一部分？ | 路由器 → Top-k 专家 → 总参数/激活参数 → 专家并行与通信 |
...
D32 已完成推理原理主线。后续主线进入 [D33～D47 推理工程实战](courses/03-inference-practice/README.md)，从环境确认、基线与压测逐步推进到优化、多卡、可靠性和综合交付；[Agent Harness 工程支线](courses/04-agent-harness/README.md)可独立从 s11 继续，不要求重学已有记录支持的 s01～s10。

| 方向 | 后续知识链 |
| --- | --- |
| 模型扩展 | 多模态编码与生成、MoE、长上下文机制 |
| 推理工程 | 推理引擎 → 压测与分析 → 量化/缓存/调度优化 → 多卡与通信 |
| Agent Harness 工程 | [Agent Loop → 工具与权限 → 上下文与记忆 → 任务与后台执行 → 团队与 MCP → 工作流和目标闭环](courses/04-agent-harness/README.md) |
| AI 平台 | 容器与 Kubernetes → GPU资源管理 → 模型部署与版本 → 监控及多租户 |
| 底层性能 | GPU架构 → 性能分析 → CUDA/Triton → 算子与通信优化 |
| 昇腾平台专项 | [Atlas 产品形态 → CANN → 训推框架 → MindStudio → 行业使能](supplements/ascend-special-topic/README.md) |

**D01～D32 的资料已生成，用户已反馈读完 D32；这只代表完成阅读，不自动代表通过自测或掌握。** 当前昇腾平台专项作为平台补充，不改变推理实战主线。Agent Harness 支线已从旧项目继承当前 17 章 LCC 快照与 s01～s10 历史进度，下一章为 s11；迁移过程未复运行代码。推理实战章节见[D33～D47 规划](courses/03-inference-practice/README.md)，全部状态见[学习进度](progress.md)。
