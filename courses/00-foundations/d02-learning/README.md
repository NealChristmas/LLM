...
上一节知道了梯度有什么用，这一节解决它怎样算出来。先不套求导规则，直接从损失公式推导出：**为什么当前 w₂ 对应的梯度是 −2？**

### 3.1 先明确：只改变哪个量？

**“损失对 w₂ 的导数”是在问：其他条件固定，只让 w₂ 改变一点，损失会怎样变？** 固定输入 x₁=1、x₂=2，目标 y=10，以及参数 w₁=2、b=1，预测便可以写成 ŷ=3+2w₂。代入平方损失，得到 **L(w₂)=½(2w₂−7)²**。这里 L(w₂) 表示在这些固定条件下，损失是一个随 w₂ 变化的函数。

我们关心的是 **w₂=3 这个位置**。此时预测为 9，损失为 0.5。下面只把 w₂ 从 3 改为 **3+h**；h 是参数的改变量，不是学习率，也不是新的参数值，可以为正或负。

### 3.2 用同一个 h，算清改动怎样传到损失

因为 w₂ 乘着输入 2，所以它改变 h，预测就改变 2h。接着将新预测代入损失公式，逐步比较：
...
最后一格用平方展开式 (a+b)²=a²+2ab+b² 展开：**(−1+2h)²=1−4h+4h²**，再乘 ½，得到新损失 **0.5−2h+2h²**。减去旧损失 0.5，就得到精确的变化量：

**损失变化 = L(3+h)−L(3) = −2h+2h²。**

到这里没有用求导技巧，只做了代入、展开和相减。它告诉我们：w₂ 改变 h，损失究竟改变多少。

### 3.3 从“变化量”得到“变化率”

损失变化 −2h+2h² 仍然取决于这次选了多大的 h。为了比较它对参数变化的敏感程度，我们用损失变化除以参数变化，得到这段改动中的**平均变化率**：

**[L(3+h)−L(3)]÷h = (−2h+2h²)÷h = −2+2h，h≠0。**

例如 h=0.01 时，比值为 −1.98；h=0.001 时，为 −1.998。当 h 从正侧或负侧越来越接近 0，这个比值都趋近于 **−2**。这个极限，就是损失在当前位置对 w₂ 的导数。**不是先令 h=0 去除以零，而是先化简，再观察 h 趋近于 0 时比值趋近于什么。**

因此，**在 w₂=3、其他量如上固定时，∂L/∂w₂=−2**。符号 ∂L/∂w₂ 读作“L 对 w₂ 的偏导数”；“偏”强调模型有多个参数，这里只改变其中一个。负号表示当前稍微增大 w₂ 会让损失下降，数值 2 表示局部变化的比例：**w₂ 改变很小的 h，损失大约改变 −2h**。

这里必须保留“大约”和“很小”：精确变化量中还有 2h²。因此，−2 不表示“w₂ 增加 1，损失一定减少 2”，也不是 w₂ 应该改成的值；它只是当前位置的局部变化率。

### 3.4 为什么原来的计算可以简写为 −1×2？

刚才把参数直接代入损失，一次算完。也可以把影响拆成 **w₂ → 预测 → 损失** 两段。第一段很直观：w₂ 改变 h，预测改变 2h，所以预测对 w₂ 的变化率为 **2**。

第二段先单独观察预测。假设预测从 9 改为 9+δ，δ 表示预测的改变量，那么新损失为 ½(−1+δ)²=0.5−δ+½δ²。减去旧损失，再除以 δ，得到 **损失变化÷预测变化=−1+½δ**；当 δ 趋近于 0，比值趋近于 **−1**。这就解释了“损失对预测的变化率为 −1”的来源，而不是要求记住一个突然出现的结论。

在本例中，δ=2h，因此两段局部变化率相乘：**损失对 w₂ 的变化率 = 损失对预测的变化率 × 预测对 w₂ 的变化率 = (−1)×2=−2**。这与直接展开算出的结果一致。把沿计算路径的变化率相乘，就是这里使用的**链式法则（Chain Rule）**。

```mermaid
flowchart LR
    W["参数 w₂"] -->|乘以输入 x₂=2| P["预测 ŷ=9"]
    P -->|与目标 y=10 比较| L["损失 L=0.5"]
    L -.->|损失对预测的变化率：−1| P
    P -.->|再乘 2，得到 w₂ 的梯度：−2| W
```

图中实线是算出预测与损失的方向，虚线是回头计算影响的方向。对于这个小公式，两种算法都很容易；对于多层网络，按计算步骤拆开、复用中间结果，就不必把整个损失展开成一个巨大的公式，也不必逐个试改参数。

同理，w₁ 乘着 x₁=1，所以它的梯度是 −1×1=−1；b 直接加到预测上，对预测的变化率为 1，因此它的梯度也是 −1。三个参数得到的梯度依次为 **(−1, −2, −1)**，并不是三个都等于预测误差。

对于多层神经网络，计算路径更长，但仍可以从损失出发，沿前向计算的关系逐层往回算；遇到多条影响路径，就把相应贡献相加。这种系统地计算参数梯度的方法叫**反向传播（Backpropagation）**。它不是把目标值倒着输入网络，也不是直接修改参数：**反向传播负责算梯度，参数更新是下一步。** [机制依据](https://docs.pytorch.org/tutorials/beginner/blitz/autograd_tutorial.html)
...
<details>
<summary><strong>卡片 1：本章例子中的损失函数有什么作用？</strong></summary>

把预测与目标的差距变成可比较的数值。采用 L=½(ŷ−y)² 时，预测越接近目标，损失越小。

</details>

<details>
<summary><strong>卡片 2：损失对某个参数的梯度分量，描述什么？</strong></summary>

其他条件固定时，该参数发生微小变化所引起的损失局部变化率；它不是预测误差，也不是该参数应该直接改动的量。

</details>

<details>
<summary><strong>卡片 3：某参数的梯度为负时，局部降低损失应往哪个方向调整？</strong></summary>

稍微增大该参数。这里是局部判断，不保证任意大的增加都能降低损失。

</details>

<details>
<summary><strong>卡片 4：反向传播与优化器分别负责什么？</strong></summary>

反向传播沿计算关系求出损失对各参数的梯度；优化器利用梯度和更新规则调整参数。

</details>

<details>
<summary><strong>卡片 5：基本梯度下降怎样使用梯度和学习率？</strong></summary>

新参数=旧参数−学习率×梯度。学习率控制步长，过大可能让损失反而增加。

</details>

<details>
<summary><strong>卡片 6：为什么更新参数后要重新计算梯度？</strong></summary>

梯度描述旧参数所在位置的局部变化率；参数改变后，预测、损失及梯度都可能改变。

</details>

<details>
<summary><strong>卡片 7：对一批样本的平均损失求梯度，是否保证每条样本都改善？</strong></summary>

不保证。各样本的调整方向可能冲突，平均梯度综合它们的影响。

</details>

<details>
<summary><strong>卡片 8：普通推理与训练在参数更新上有什么区别？</strong></summary>

普通推理用已有参数计算结果；训练还要利用损失、梯度和优化器更新参数。
