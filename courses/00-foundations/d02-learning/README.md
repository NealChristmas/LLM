...
沿用 D01 的计算。D01 只讨论模型输出，所以把它写成 y；现在需要同时比较“模型预测”和“真实目标”，因此改用 **ŷ（读作 y 帽）表示预测，用 y 表示目标**。输入为 x₁=1、x₂=2，模型按 **ŷ=w₁x₁+w₂x₂+b** 计算，其中 w₁、w₂ 是权重，b 是偏置，三者都是可调整的参数。现在 w₁=2、w₂=3、b=1，所以预测为 2×1+3×2+1=9；这条训练数据给出的目标 y=10。
...
**w₂=3 处的 ∂L/∂w₂ = lim[h→0] [L(3+h)−L(3)]/h**
