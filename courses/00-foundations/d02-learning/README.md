...
上面的试改只是帮助理解。对于大量参数，逐个尝试会很费事；我们可以利用已有的计算关系，把“参数怎样影响预测”和“预测怎样影响损失”连起来。以 w₂ 为例：w₂ 先通过乘以 x₂ 影响预测，预测再通过平方损失影响 L。
...
实线是算出预测与损失的方向，虚线是回头计算影响的方向。对于本章选定的损失 L=½(ŷ−y)²，损失对预测的变化率恰好是 ŷ−y，现在等于 −1；而预测对 w₂ 的变化率是 x₂=2。沿这条关系相乘，得到 w₂ 的梯度为 **−1×2=−2**。这种把一段段变化率连乘起来的规则叫**链式法则（Chain Rule）**。暂时不必练习求导技巧，先理解它在追踪“一个改动经过各步计算，最终怎样影响损失”。
