...
模型权重可以看成大量已经训练好的数字。服务启动时，程序读取这些数字并把它们放入 GPU 显存；请求到达后，Tokenizer 先在 CPU 一侧把文字变成 Token ID，随后这些 ID 对应的向量也被送入 GPU。此时显存里虽然有权重和输入，却还没有产生答案，因为硬件还需要收到“对哪些数据执行什么运算”的指令。
