...
引入一个更便宜的**草稿模型（draft model）**，先连续猜出若干候选 Token。假设当前文本是“法国的首都是”，草稿模型依次猜出“巴”“黎”“市”。目标大模型随后在一次并行前向中检查这些位置。这个机制称为**推测解码（speculative decoding）**。

```mermaid
flowchart LR
    A["当前文本：法国的首都是"] --> B["草稿依次猜：巴、黎、市"]
    B --> C["目标模型一次前向<br/>同时计算三个候选位置"]
    C --> D["接受：巴、黎"]
    C --> E["拒绝：市"]
    E --> F["按验证规则改为：。"]
    D --> G["本轮推进多个 Token"]
...
这一轮结束后得到“法国的首都是巴黎。”，下一轮从句号之后继续。草稿模型只负责提出建议，最终结果由目标模型的验证规则控制。为了建立机制直觉，可以先把它理解为“接受目标模型认可的连续前缀，在第一个分歧处修正”；但严谨的随机采样算法不是简单比较两个模型各自概率最大的 Token，而会根据草稿概率与目标概率执行接受、拒绝和概率校正，从而保持目标模型应有的输出分布。

## 2. 为什么目标模型能并行验证？

草稿已经给出了“巴、黎、市”这段连续输入，因此目标模型可以像处理一小段 Prefill 一样并行计算三个位置：检查“法国的首都是”之后能否接受“巴”，检查“法国的首都是巴”之后能否接受“黎”，再检查“法国的首都是巴黎”之后能否接受“市”。**因果遮罩保证每个位置只能看到它前面的上下文和草稿前缀**，所以这些位置虽然在一次前向中并行计算，条件关系仍与逐个生成一致。

若前两个候选被接受，这一次目标模型调用就至少向前推进两个候选 Token，而普通 Decode 通常需要两次串行调用。某些算法在所有候选都被接受时，还能利用验证结果再产生一个目标 Token；具体推进数量取决于算法实现，本章只抓住“昂贵的目标模型一次验证多个位置”这一共同机制。

但验证仍要计算这些候选，草稿模型本身也有成本。只有“减少的目标模型串行轮数”大于“草稿生成与验证的额外工作”，才会获得加速。

## 3. 接受率为什么决定收益？

若草稿模型经常与目标模型一致，一轮猜 4 个可能接受 3～4 个；若经常在第一个就分歧，后面的候选虽然已经参与验证，却不能直接沿用，额外工作被浪费。平均每轮被接受的候选数可用**接受长度**描述，它与草稿质量、任务、采样参数和候选长度有关。接受率描述候选中有多少比例被接受，接受长度则更直接地回答“一次目标模型验证平均让序列前进多少”；评估端到端收益时不能只看其中一个比例。
