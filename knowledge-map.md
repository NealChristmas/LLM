...
**先看领域全景，再沿章节逐步补全。** 当前采用“正文与图示 → 助记卡片 → 自测题”，不做实验。D01～D17 是章节编号；每天约两章，第一轮暂按 8～10 个学习日安排。你已读完 D01 正文，接下来是 D02；读完和通过自测分别记录在[学习进度](progress.md)。
...
**下一块拼图是 D02：我们已经知道如何用参数算出预测，接下来要解释怎样根据误差修改参数。** 章节学习方式见[学习路线](roadmap.md)，完成情况见[学习进度](progress.md)。
